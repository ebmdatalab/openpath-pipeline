...
class RowAnonymiser:
    def __init__(self, lab, filters, normaliser, log_level=None):
        self.orig_row = None
        self.row = None
    ...
    def process_row(self):
        try:
            for row_filterer in self.filters:
                row_filterer(self.row)
            self.normaliser(self)
...
def process_file(lab, row_iterator, filters, normaliser, filename):
    anonymiser = Anonymiser(
        lab, row_iterator=row_iterator, filters=filters, normaliser=normaliser
    )
...
def process_files(
    lab, filenames, row_iterator, filters, normaliser, multiprocessing=False
):
    filenames = sorted(filenames)
    process_file_partial = partial(process_file, lab, row_iterator, filters, normaliser)
    if multiprocessing:
        with Pool() as pool:
